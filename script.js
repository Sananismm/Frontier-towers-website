...
const closeMenu = () => { if (!header?.classList.contains('menu-open')) return; header.classList.remove('menu-open'); document.body.classList.remove('body-menu-open'); menuToggle?.setAttribute('aria-expanded', 'false'); menuToggle?.setAttribute('aria-label', 'Open menu'); };
const updateHeader = () => header?.classList.toggle('scrolled', window.scrollY > 24);
updateHeader(); window.addEventListener('scroll', updateHeader, { passive: true });
menuToggle?.addEventListener('click', () => { const open = header.classList.toggle('menu-open'); document.body.classList.toggle('body-menu-open', open); menuToggle.setAttribute('aria-expanded', String(open)); menuToggle.setAttribute('aria-label', open ? 'Close menu' : 'Open menu'); if (open) nav?.querySelector('a')?.focus(); });
...
const demandedService = servicesTrack?.querySelector('a[href*="remote-monitoring-system.html"]');
if (demandedService) { demandedService.classList.add('service-card--featured'); if (servicesTrack.firstElementChild !== demandedService) servicesTrack.prepend(demandedService); }
...
// The canonical service list lives in services-data.js. Static options remain in
// the HTML so the form is still usable when JavaScript is unavailable.
const serviceSelect = document.querySelector('select[name="service"]');
if (serviceSelect && Array.isArray(window.FRONTIER_SERVICES)) {
  const currentValue = serviceSelect.value;
  serviceSelect.replaceChildren(new Option('Choose a service', ''));
  window.FRONTIER_SERVICES.forEach(([, name, , slug]) => serviceSelect.add(new Option(name, slug.replace('.html', ''))));
